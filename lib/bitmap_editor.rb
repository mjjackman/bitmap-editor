...
class BitmapEditor
  ...
  def run(file_name)
    ...
    file.each do |line|
      ...
      when 'V'
        if params_order(command: 'V', params_num: array_line.length, a: array_line[1].to_i, b: array_line[2].to_i, c: array_line[3].to_i) && check_params(line)
          @bitmap.colour_vertical_px(array_line[1].to_i, array_line[2].to_i, array_line[3].to_i, array_line[4])
        else
          return invalid_msg(line)
        end
      when 'H'
        if params_order(command: 'H', params_num: array_line.length, a: array_line[1].to_i, b: array_line[2].to_i, c: array_line[3].to_i) && check_params(line)
